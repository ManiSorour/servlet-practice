...
public class ProductServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        BufferedReader reader = request.getReader();   // همون بخش request body
        Product writedNewProduct = gson.fromJson(reader, Product.class);

        User performedBy = new Admin(1, "test_admin", "pw");


        try {
            wareHouseService.addProduct(
                    writedNewProduct.getName(),
                    writedNewProduct.getCode(),
                    writedNewProduct.getCategory(),
                    writedNewProduct.getPurchasePrice(),
                    writedNewProduct.getSellPrice(),
                    writedNewProduct.getQuantity(),
                    5,
                    performedBy
            );
